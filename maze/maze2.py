...
pygame.display.set_caption("Exact Map with Reduced Screen Shake")
...
# --- Base Layout (Matching Video Exactly) ---
walls = []
...
# Inner Zig-Zag Walls (Leaves 80px gaps)
walls.append(create_wall(20,  100, 310, 30)) 
walls.append(create_wall(100, 190, 310, 30)) 
walls.append(create_wall(20,  280, 310, 30)) 
walls.append(create_wall(100, 370, 310, 30)) 
walls.append(create_wall(20,  460, 310, 30)) 
walls.append(create_wall(100, 550, 310, 30)) 
walls.append(create_wall(20,  640, 310, 30)) 

# --- Sequential Closing Slabs ---
class ClosingSlab:
    ...
    def draw(self, surface, offset):
        ...
        pygame.draw.rect(surface, SLAB_GREEN, rect)
        pygame.draw.rect(surface, (0, 0, 0), rect, 2)
        pygame.draw.rect(surface, (190, 240, 130), (rect.x + 2, rect.y + 2, rect.w - 4, 8))

slabs = []
prev_slab = None

# TARGETS FIXED TO CLOSE COMPLETELY AGAINST THE WALLS
slab_configs = [
    (40,  60, -200, 210),  # Top slab seals flush to the right wall (x=410)
    (130, 60,  630, 220),  # Seals flush to the left wall (x=20)
    (220, 60, -200, 210),  
    (310, 60,  630, 220),  
    (400, 60, -200, 210),  
    (490, 60,  630, 220),  
    (580, 60, -200, 210),
    (670, 60,  630, 340)   # Final slab stops flush against the Breakable Gate Block
]
...
block_body.position = (125, 700) # Centered perfectly in the gap before the finish line
...
for i, color in enumerate(BALL_COLORS):
    ...
    # ALL balls start at the exact same location
    body.position = (SPAWN_X, SPAWN_Y)
    
    # Tiny initial spread velocity so they separate from each other instantly
    ...
    # Added "vertical_bounces" counter to track loops
...
def collision_handler(arbiter, _space, _data):
    ...
    if ball_shape:
        for ball in balls:
            if ball["shape"] == ball_shape:
                ...
                # 1. Add organic randomness to EVERY collision
                ...
                # 2. Check if collision is heavily vertical (flat floor/ceiling hit)
                if abs(normal.x) < 0.2:
                    ball["vertical_bounces"] += 1
                else:
                    ball["vertical_bounces"] = 0

                # 3. If maximum 5 vertical collisions reached, FORCE a large horizontal direction change
                if ball["vertical_bounces"] >= 5:
                    direction = random.choice([-1, 1])
                    kick_x = direction * random.uniform(150, 300)  # Massive horizontal kick
                    ball_shape.body.velocity = pymunk.Vec2d(current_vel.x + kick_x, current_vel.y)
                    ball["vertical_bounces"] = 0  # Reset counter
    ...
    # REDUCED SCREEN SHAKE for high-impact bounces (max 4 pixels instead of 10)
    if strength > 300:
...
def block_hit_handler(arbiter, _space, _data):
    ...
    # Optional randomness for block hits too
    ball_shape = next((shape for shape in arbiter.shapes if shape.collision_type == BALL_COLLISION), None)
    if ball_shape:
        current_vel = ball_shape.body.velocity
        ball_shape.body.velocity = pymunk.Vec2d(
    ...
    # Decrease block health upon hard impacts
    if strength > 5 and block_health > 0:
        block_health -= 1
        screen_shake = max(screen_shake, 2) # REDUCED from 4 to 2 for subtle impact bump
        point = arbiter.contact_point_set.points[0].point_a
...
# Pymunk 7 requires on_collision() instead of add_collision_handler()
...
while running:
    ...
    if block_active and block_health <= 0:
        # Shatter the block, allowing balls into the finish line
        space.remove(block_body, block_shape)
        block_active = False
        play_impact_sound(800)
        screen_shake = 7 # REDUCED from 15 to 7 for shatter explosion
        for _ in range(50):
    ...
    if winner_found:
        ...
        # Pull ball into the finish line box
        if winner_ball:
    ...
    for ball in balls:
        ...
        # Trigger Win Condition (Requires passing where the gate used to be, x < 100)
