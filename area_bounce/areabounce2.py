...
pygame.display.set_caption("Glacial Frost Gravity Bounce (Constant Size)")
...
# =====================================================
# COLORS (Glacial Frost Aesthetic)
# =====================================================
BG = (2, 8, 20)               # Deep midnight abyss
FROST_WHITE = (200, 240, 255) # Frost white (for main text)
DIM_ICE = (100, 150, 180)     # Dimmed blue for watermark/subtext
PURE_CYAN = (0, 255, 255)     # Bright cyan for highlights

# Icy Blues, Cyans, and Pure White
STRIPE_COLORS = [
    (0, 50, 255, 255),     # Deep Ocean Blue
    (0, 120, 255, 255),    # True Blue
    (0, 200, 255, 255),    # Bright Cyan
    (150, 230, 255, 255),  # Pale Ice
    (255, 255, 255, 255),  # Pure Snow White
]
...
# BALL & PHYSICS PARAMS
# =====================================================
BALL_RADIUS = 24.0        # Constant size restored
PAINT_BLEED = 3.0
GRAVITY = 1500            
MAX_SPEED = 2200          
BOUNCE_BOOST = 1.01       
...
def color_from_hue(hue):
    ...
    # Restricted hue between 190 (Cyan) and 250 (Deep Blue)
    restricted_hue = 190 + (hue % 60)
    color.hsla = (restricted_hue, 90, 65, 100)
    return color
...
# Generate the texture once since size no longer changes
...
while running:
    ...
    if game_state == "PLAYING":
        ...
        if distance >= inner_limit:
            ...
            # Paint precisely to the edge of the impact
            if last_stamp_pos is not None:
                paint_stroke(last_stamp_pos, impact_pos, BALL_RADIUS)
            ...
            if last_stamp_pos is None:
                ...
                paint_stroke(last_stamp_pos, next_pos, BALL_RADIUS)
    ...
    # Hook Text (Top)
    hook_text = hook_font.render("WILL IT FILL 100%?", True, FROST_WHITE)
    hook_rect = hook_text.get_rect(center=(WIDTH // 2, 65))
    screen.blit(hook_text, hook_rect)

    # Fill Percentage Formatting (No leading zero unless it's exactly 100%)
    ...
    fill_text = counter_font.render(f"Filled: {formatted_percent}", True, PURE_CYAN)
    fill_rect = fill_text.get_rect(
        center=(WIDTH // 2, HEIGHT // 2 + CIRCLE_RADIUS + 35)
    )
    screen.blit(fill_text, fill_rect)

    # Bounce Counter (Directly below Fill Percentage)
    bounces_text = counter_font.render(f"Bounces: {collision_count}", True, FROST_WHITE)
