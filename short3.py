...
# =====================================================
try:
    bounce_sound = pygame.mixer.Sound("sounds/short3.mp3")
except Exception:
    print("Warning: '17800745449874VMz2Vl0.mp3' not found. Make sure it is in the same folder as this script!")
    bounce_sound = None
...
while running:
    ...
    watermark_font = pygame.font.SysFont(None, 28) # Smaller font for the watermark
    
    if game_state == "PLAYING":
        
        # 1. THE BOUNCE COUNTER (Moved above the circle)
        bounces_text = font.render(f"Bounces: {collision_count}", True, (255, 255, 255))
        # Placed dynamically based on the circle's top edge
        bounces_rect = bounces_text.get_rect(center=(WIDTH // 2, HEIGHT // 2 - CIRCLE_RADIUS - 60))
        screen.blit(bounces_text, bounces_rect)

        # 2. THE WATERMARK (Dim grey, placed below the circle)
        watermark_text = watermark_font.render("@BounceCult", True, (120, 120, 120))
        # Placed dynamically based on the circle's bottom edge
        watermark_rect = watermark_text.get_rect(center=(WIDTH // 2, HEIGHT // 2 + CIRCLE_RADIUS + 60))
        screen.blit(watermark_text, watermark_rect)
        
    else:
        # Final explosion screen text
